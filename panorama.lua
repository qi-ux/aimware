local _INFO,
    ...
    DllImport,
    UIEngine,
    nativeIsValidPanelPointer,
    nativeGetLastDispatchedEventTargetPanel,
    nativeCompileRunScript,
    nativeRunScript,
    nativeGetV8GlobalContext,
    nativeGetIsolate,
    nativeGetParent,
    nativeGetID,
    nativeFindChildTraverse,
    nativeGetJavaScriptContextParent,
    nativeGetPanelContext,
    jsContexts,
    getJavaScriptContextParent,
    v8_dll,
    persistentTbl,
    Local,
    MaybeLocal,
...
ffi.cdef [[
        ...
        bool K32GetModuleInformation(void*, void*, void*, unsigned long);
    ]]

local MODULEINFO = ffi.typeof [[
        struct {
            void* lpBaseOfDll;
...
local function find_signature(module_name, signature)
    local hHandle = ffi.C.GetModuleHandleA(module_name)
    if hHandle == nil then
        return nil
    end

    local lpModuleInfo = MODULEINFO()
    if not ffi.C.K32GetModuleInformation(ffi.C.GetCurrentProcess(), hHandle, lpModuleInfo, ffi.sizeof(MODULEINFO)) then
        return nil
    end

    local image_bytes = ffi.cast(uint8_t_ptr, hHandle)

    for i = 0, lpModuleInfo.SizeOfImage - #signature - 1 do
        local found = true
...
find_pattern = find_signature
create_interface = function(module_name, interface_name)
    return proc_bind(module_name, "CreateInterface", "void*(*)(const char*, int*)")(interface_name, nil)
end

safe_mode = xpcall and true or false
print(("\nluv8 panorama library;\napi: %s;\nenabled features: safe_mode: %s; rawops: %s; ffi.C: %s"):format("aimware", tostring(safe_mode), tostring(rawget ~= nil), tostring(ffi.C ~= nil)))
_error = error
if 1 + 2 == 3 then
    error = function(msg)
...
proc_bind =
    ...
    if ffi.C then
        ...
        fnGetProcAddress =
            cast("uint32_t(__stdcall*)(uint32_t, const char*)", cast("uint32_t**", cast("uint32_t", find_pattern("engine.dll", "\xFF\x15\xCC\xCC\xCC\xCC\xA3\xCC\xCC\xCC\xCC\xEB\x05")) + 2)[0][0])
        fnGetModuleHandle = cast("uint32_t(__stdcall*)(const char*)", cast("uint32_t**", cast("uint32_t", find_pattern("engine.dll", "\xFF\x15\xCC\xCC\xCC\xCC\x85\xC0\x74\x0B")) + 2)[0][0])
    end
...
nativeRunScript =
    __thiscall(
    cast(
        typeof("void*(__thiscall*)(void*,void*,void*,void*,int,bool)"),
        follow_call(find_pattern("panorama.dll", api == "legendware" and "\xE8\xCC\xCC\xCC\xCC\x8B\x4C\x24\x10\xFF\x15\xCC" or "\xE8\xCC\xCC\xCC\xCC\x8B\x4C\x24\x10\xFF\x15\xCC\xCC\xCC\xCC"))
    ),
...
nativeGetPanelContext = __thiscall(cast("void***(__thiscall*)(void*,void*)", follow_call(find_pattern("panorama.dll", "\xE8\xCC\xCC\xCC\xCC\x8B\x00\x85\xC0\x75\x1B"))), UIEngine:getInstance())
jsContexts = {}
...
do
    ...
    local _base_0 = {
        ...
        callAsFunction = function(self, recv, argc, argv)
            return MaybeLocal(
                v8_dll:get("?CallAsFunction@Object@v8@@QAE?AV?$Local@VValue@v8@@@2@V32@HQAV32@@Z", "void*(__thiscall*)(void*,void*,void*,int,void*)")(self.this, intbuf, recv, argc, argv)
            )
        end,
        getIdentityHash = function(self)
            return v8_dll:get("?GetIdentityHash@Object@v8@@QAEHXZ", "int(__thiscall*)(void*)")(self.this)
...
do
    ...
    local _base_0 = {
        fromLua = function(self, isolate, val)
            local arr =
                Array(MaybeLocal(v8_dll:get("?New@Array@v8@@SA?AV?$Local@VArray@v8@@@2@PAVIsolate@2@H@Z", "void*(__cdecl*)(void*,void*,int)")(intbuf, isolate, #val)):toLocalChecked()():getInternal())
            for i = 1, #val do
            ...
            __init = function(self, callback)
                self.this =
                    ...
                    v8_dll:get(
                        ...
                        "void*(__cdecl*)(void*,void*,void*,void*,void*,int,int)"
                    )(intbuf, nativeGetIsolate(), callback, new("int[1]"), new("int[1]"), 0, 0)
                ):toLocalChecked()
            end,
            __base = _base_0,
    ...
    _class_0 =
        setmetatable(
        {
            __init = function(self, isolate, bool)
                self.this =
                    Value(
                    cast("uintptr_t", isolate) + ((function()
                            if bool then
                                return 0x4C
                            else
                                return 0x50
    ...
    _class_0 =
        setmetatable(
        {
            __init = function(self, isolate, val)
                self.this =
                    MaybeLocal(v8_dll:get("?New@Number@v8@@SA?AV?$Local@VNumber@v8@@@2@PAVIsolate@2@N@Z", "void*(__cdecl*)(void*,void*,double)")(intbuf, isolate, tonumber(val))):toLocalChecked()
            end,
            __base = _base_0,
            __name = "Number",
            __parent = _parent_0
...
do
    ...
    _class_0 =
        setmetatable(
        {
            __init = function(self, isolate, val)
                self.this =
                    MaybeLocal(v8_dll:get("?NewFromUnsigned@Integer@v8@@SA?AV?$Local@VInteger@v8@@@2@PAVIsolate@2@I@Z", "void*(__cdecl*)(void*,void*,uint32_t)")(intbuf, isolate, tonumber(val))):toLocalChecked(
    ...
    _class_0 =
        setmetatable(
        {
            __init = function(self, isolate, val)
                self.this =
                    MaybeLocal(
                    v8_dll:get("?NewFromUtf8@String@v8@@SA?AV?$MaybeLocal@VString@v8@@@2@PAVIsolate@2@PBDW4NewStringType@2@H@Z", "void*(__cdecl*)(void*,void*,const char*,int,int)")(
                        intbuf,
                        isolate,
                        val,
                        0,
...
do
    ...
    local _base_0 = {
        compile = function(self, panel, source, layout)
            ...
            return __thiscall(
                cast(
                    "void**(__thiscall*)(void*,void*,const char*,const char*)",
                    api == "memesense" and find_pattern("panorama.dll", "\xE8\xCC\xCC\xCC\xCC\x8B\x4C\x24\x10\xFF\x15\xCC\xCC\xCC\xCC") - 2816 or
                        find_pattern("panorama.dll", "\x55\x8B\xEC\x83\xE4\xF8\x83\xEC\x64\x53\x8B\xD9")
                ),
                UIEngine:getInstance()
            )(panel, source, layout)
        end,
